...
//------INSERT-----PRODUCTS---------------------------------------------------------------------------------------------
    ...
    public String addProducts(@RequestParam int id_subcategory, @RequestParam String productName,
        ...
        redirectAttributes.addFlashAttribute("message", "Produit "+ productName + " a bien été créé");
...
//--!----UPDATE-----PRODUCTS--------------------------------------------------------------------------------------------
    ...
    public String updateProducts(@RequestParam int productid, @RequestParam int id_subcategory, @RequestParam(required = false) String productName,
                                 @RequestParam(required = false) String productdescription, @RequestParam(required = false) Double productprice, @RequestParam(required = false) Integer productstock,
                                 @RequestParam(required = false) MultipartFile file, Model model, RedirectAttributes redirectAttributes) throws SQLException {
        if (productid <= 0 || id_subcategory <= 0) {
            throw new IllegalArgumentException("Invalid product or subcategory ID");
        }
...
//        model.addAttribute("idproduct", productid);
        ...
        BdProducts db = new BdProducts();
        db.updateProduct(productid,id_subcategory,website_name,productName,productdescription,productprice,productstock,imageUrl);
        redirectAttributes.addFlashAttribute("message", "Produit "+ productName + " a bien été modifié");
        return "redirect:/shopkeeper/dashboard";
    }
    ...
    public String deleteProduct(@RequestParam int productid, @RequestParam String productName,
        ...
        model.addAttribute("website_name", website_name);
        redirectAttributes.addFlashAttribute("website_name", website_name);
        redirectAttributes.addFlashAttribute("error", "Produit "+ productName + " a bien été supprimé");
